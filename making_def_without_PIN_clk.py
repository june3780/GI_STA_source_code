...
def get_temp_buffer_def(defname, wire, file_type):
    file_address='../data/deflef_to_graph_and_verilog/with_CTS_buffer/'+file_type
    if file_type not in os.listdir('../data/deflef_to_graph_and_verilog/with_CTS_buffer/'):
    ...
    if defname.split('.def')[0]+'.json' in os.listdir('../data/deflef_to_graph_and_verilog/results/'+file_type+'/test_7800_zfor_clk_'+wire+'_with_skew'+'/'):
        address_of_cts='../data/deflef_to_graph_and_verilog/results/'+file_type+'/test_7800_zfor_clk_'+wire+'_with_skew'+'/'+defname.split('.def')[0]+'.json'
        ...
        for idx, line in enumerate(file):
            ...
            if idx<whereisclk or idx>=min_idx:
                if idx==0:
                    f=open(will_be_change,'w')
                    f.write(line)
                    f.close()

                else:
                    f=open(will_be_change,'a')
                    f.write(line)
                    f.close()
        file.close()
